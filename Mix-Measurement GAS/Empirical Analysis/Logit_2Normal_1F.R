#### Purpose: Calibrate the logit-normal GAS model with a pool of loans and 2 macro variables
...
##Conclusions:
#1. Similar to the model with one macro-factor, the model can replicate the observed default series accurately.
...
#4. Provided that the results of the Logit_1Normal_1F and Logit_2Normal_1F are the same, it is clear the extension to the 
#   multivariate setting for the normal component has been performed appropiately.
#5. The constraints imposed on the parameters during the optimization procedure are of outmost importance for the 
#   optimization to converge. 
#6. As the number of parameters increases, parameter constraints become more important.
