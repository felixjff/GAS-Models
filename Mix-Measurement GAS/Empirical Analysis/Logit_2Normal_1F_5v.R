#### Purpose: Calibrate the logit-normal GAS model with a pool of loans, 2 macro variables, one factor and 
...
loglikelihood <- function(par, path_l, path_n, path_l_cscore, path_l_oltv, path_l_dti, path_l_origamt, path_l_lastrt, tobs, cobs){
  ...
  for(i in 1:length(tobs)){
    ...
    score_n_[i] <- t(Zm)%*%Siginv%*%(path_n[i,] - Zm*f_[i])
...
#Initialization
parameters <- c(Zm1 = 0.1, Zm2 = 0.5, Zc = 0.001, B = 0.8, A = 0.5, Sig1 = 0.09, Sig2 = 0.09, f1 = 3, Beta = c(1, 1, 1, 1, 1) , w = 0)
...
par <- c(fit$par[1],  fit$par[2], fit$par[3], abs(fit$par[4]), abs(fit$par[5]), abs(fit$par[6]), abs(fit$par[7]), abs(fit$par[8]), abs(fit$par[9]),
